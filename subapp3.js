...
// ---------- RTDB refs (original names) ----------
const entriesRef    = () => ref(rtdb, "tn_entries");
const fieldsRef     = () => ref(rtdb, "tn_fields");
const headOfficeRef = () => ref(rtdb, "tn_settings/headOffice");
...
const paymentPrevInput = document.getElementById("paymentPrev");
const paymentOnInput = document.getElementById("paymentOn");
const paymentUptoInput = document.getElementById("paymentUpto");
const paymentBalancePctInput = document.getElementById("paymentBalancePct");
const allotmentBalancePctInput = document.getElementById("allotmentBalancePct");
...
function calcAutoFields() {
  ...
  const payPrev = parseFloat(paymentPrevInput.value) || 0;
  const payOn   = parseFloat(paymentOnInput.value) || 0;
  const payUpto = payPrev + payOn;
  paymentUptoInput.value = payUpto.toFixed(2);

  let payPct = 0;
  if (payPrev > 0) {
    const remaining = payPrev - payUpto;
    payPct = (remaining / payPrev) * 100;
  }
  paymentBalancePctInput.value = payPct.toFixed(2);

  let allotPct = 0;
  if (allot > 0) {
    allotPct = (bal / allot) * 100;
  }
  allotmentBalancePctInput.value = allotPct.toFixed(2);
...
paymentPrevInput.addEventListener("input", calcAutoFields);
paymentOnInput.addEventListener("input",  calcAutoFields);
...
function renderTable(list) {
  ...
  list.forEach(e => {
    ...
    tr.innerHTML = `
      ...
      <td>${(e.paymentPrev || 0).toFixed(2)}</td>
      <td>${(e.paymentOn || 0).toFixed(2)}</td>
      <td>${(e.paymentUpto || 0).toFixed(2)}</td>
      <td>${(e.paymentBalancePct || 0).toFixed(2)}</td>
      <td>${(e.allotmentBalancePct || 0).toFixed(2)}</td>
      <td>
...
function autoFillFromLastEntry() {
  ...
  if (!dist || !field) {
    ...
    paymentPrevInput.value = 0;
    paymentOnInput.value = 0;
  ...
  if (!previous) {
    ...
    html = `
      ...
      Enter Allotment, Previous Day Balance, Payment and On The Day manually.
    `;
  } else {
    allotmentInput.readOnly = true;
    openingBalanceInput.readOnly = true;
    ...
    paymentPrevInput.value = previous.paymentUpto || 0;
    paymentOnInput.value = 0;
    ...
    html = `
      ...
      <div><strong>Previous Day Balance:</strong> ${previous.uptoTheDay.toFixed(2)} (last Upto)</div>
...
async function saveHeadOfficeToDB(value) {
  await set(headOfficeRef, value);
}
async function clearHeadOfficeInDB() {
  await remove(headOfficeRef);
}
...
function exportToExcel() {
  ...
  const data = [
    ["Date","District","Field","Allotment","Prev Bal","On The Day",
     "Upto The Day","Balance","Pay Prev","Pay On","Pay Upto","Pay Bal %","Allot Bal %"]
  ];
  rows.forEach(e => {
    data.push([
      e.date,
      ...
      e.balance.toFixed(2),
      (e.paymentPrev || 0).toFixed(2),
      (e.paymentOn || 0).toFixed(2),
      (e.paymentUpto || 0).toFixed(2),
      (e.paymentBalancePct || 0).toFixed(2),
      (e.allotmentBalancePct || 0).toFixed(2)
    ]);
  });
    ...
    <table border="1" cellpadding="6" cellspacing="0" style="width:100%;border-collapse:collapse;">
      <thead>
        <tr style="background:#f0f0f0;">
          ...
          <th>Prev Bal</th><th>On The Day</th><th>Upto The Day</th><th>Balance</th>
          <th>Pay Prev</th><th>Pay On</th><th>Pay Upto</th><th>Pay Bal %</th><th>Allot Bal %</th>
        </tr>
      </thead>
      <tbody>
        ${rows.map(e => `
          <tr>
            ...
            <td align="right">${(e.paymentPrev || 0).toFixed(2)}</td>
            <td align="right">${(e.paymentOn || 0).toFixed(2)}</td>
            <td align="right">${(e.paymentUpto || 0).toFixed(2)}</td>
            <td align="right">${(e.paymentBalancePct || 0).toFixed(2)}</td>
            <td align="right">${(e.allotmentBalancePct || 0).toFixed(2)}</td>
          </tr>`).join("")}
      </tbody>
    </table>
  `;
...
entryForm.addEventListener("submit", async e => {
  ...
  const paymentPrev = parseFloat(paymentPrevInput.value) || 0;
  const paymentOn   = parseFloat(paymentOnInput.value) || 0;
  const paymentUpto = paymentPrev + paymentOn;
  let paymentBalancePct = 0;
  if (paymentPrev > 0) {
    const remaining = paymentPrev - paymentUpto;
    paymentBalancePct = (remaining / paymentPrev) * 100;
  }
  let allotmentBalancePct = 0;
  if (allotment > 0) {
    allotmentBalancePct = (balance / allotment) * 100;
  }
...
tableBody.addEventListener("click", async e => {
  ...
  if (btn.classList.contains("editBtn")) {
    ...
    paymentPrevInput.value = entry.paymentPrev || 0;
    paymentOnInput.value = entry.paymentOn || 0;
